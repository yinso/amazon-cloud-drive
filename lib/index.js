...
(function() {
  var CloudDrive, EventEmitter, app, debug, express, opener, request, url,
  ...
  CloudDrive = (function(superClass) {
    ...
    CloudDrive.prototype.authenticate = function(cb) {
      var self;
      self = this;
      return self.redirect(function(err, res) {
        if (err) {
          return cb(err);
        } else {
          return self.getOAuthToken(res, function(err, result) {
            if (err) {
              return cb(err);
            } else {
    ...
    CloudDrive.prototype.redirect = function(cb) {
      ...
      app.get(callbackUri.pathname, function(req, res, next) {
        if (req.query.error) {
          ...
          self.once('authenticated', function(result) {
            return res.json(result);
          });
          self.once('error', function(err) {
            return res.status(400).json(err);
          });
          return cb(null, req.query);
        }
      });
